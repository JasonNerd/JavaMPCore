...
## `volatile`关键字: **保证变量可见性.**
可见, 是指不同线程间该变量均可见, 也即对于各线程而言该变量应该是始终一致的, 不存在一个线程修改该变量而其他线程不可见(也即不清楚变量已修改仍见到旧值)的情况.

是什么样的原因造成将JVM 设置为-server 时就出现死循环呢?在启动 RunThread.java 线程时，变量 `private boolean isRunning = true;`存在于公共堆栈及线程的私有堆栈中。在JVM 被设置为 -server 模式时为了线程运行的效率，线一直在私有堆栈中取得isRunning 的值是 true。而代码 `thread.setRunning(false);`虽然被执行，更新的却是公共堆栈中的isRunning变量值 false，所以一直就是死循环的状态.

关键字 volatile 提示线程每次从共享内存中读取变量, 而不是从私有内存中读取, 这样就保证了同步数据的可见性, 如果修改实例变量中的数据，比如 i++, 也就是i+1, 则这样结果的操作其实并不是一个原子操作, 也就是非线程安全的.
![](image/2023-09-05-13-24-08.png)

除了在i++操作时使用synchronized 关键字实现同步外，还可以使用AtomicInteger 原子类进行实现。原子操作是不能分割的整体，没有其他线程能够中断或检查正在原子操作中的变量。一个原子 (atomic)类型就是一个原子操作可用的类型，它可以在没有锁的情况下做到线程安全(thread-safe)
